   				...
   				$.each(response, function(key, value) { //populate child options
   					...
   					var eachrow = "<tr>"
   						+ "<td><input type='checkbox' /> </td>"
   		                 + "<td>" + value.Cr_Id+"</td>"
   		                 + "<td>" + value.Cr_Date + "</td>"
   		                 + "<td class='more'>" + value.Cr_Name + "</td>"
   		                 + "<td>" + value.Cr_Withdraw + "</td>" 
   		              	+ "<td>" + value.Cr_A_Balance + "</td>" 
   		           		+ "<td>" + value.Cr_C_Balance + "</td>" 
   		        		+ "<td class='more'>" + value.Cr_Description + "</td>" 
   		                 + "</tr>";
   		              
   					$("#creditRecord").append(eachrow);
      ...
      <tr>
        <td>
          <div class="headTableFloat">
              <strong>Creditors Records</strong>
            	<div><button type="submit"  onclick="return Checkboxchek();" class="GrayBtn GrayBtn-primary">Delete</button></div>
              	<div><button class="GrayBtn GrayBtn-primary" onclick="return openCredit();">Add</button></div>
              	<div><button class="GrayBtn GrayBtn-primary" onclick="return SelectMailDynamic('Direct', 'CreditRecord');">Send email</button></div>
              	<div><button class="GrayBtn GrayBtn-primary" onclick="return commonPOPUpTwoDates('CreditRecord');">Excel</button></div>
          </div>
        </td>
      </tr>
      <tr>
        <td>
            <table border="1" id="example" class="display table table-striped table-bordered"  cellspacing="0" width="100%">
              ...
              <tr>
                ...
                <th style="border-right-style: hidden;">Deposite</th>
                <th style="border-right-style: hidden;">Available Balance</th>
                <th style="border-right-style: hidden;">Current Balance</th>
                <th style="border-right-style: hidden;">Description</th>
                </tr>
              </thead>
              <tfoot>
    			<tr><th></th> <th></th> <th></th> <th>total: </th> <th></th> <th></th> <th></th> <th></th></tr>
    			</tfoot>
              <tbody id="creditRecord">
