...
function openDebitPage(){
		window.open("./openDebitList", "_self");
}
   				...
   				$.each(response, function(key, value) { //populate child options
   					...
   					var eachrow = "<tr>"
   						+ "<td><input type='checkbox' /> </td>"
   		                 + "<td>" + value.debitClear_Id+"</td>"
   		                 + "<td>" + value.debitClear_Date + "</td>"
   		                 + "<td class='more'>" + value.debitClear_name + "</td>"
   		                 + "<td>" + value.debitClear_Amount + "</td>"
   		              	 + "<td>" + value.debitClear_balance + "</td>"
   		           		 + "<td class='more'>" + value.debitClear_Source + "</td>"
   		           		+ "<td>" + value.debitClear_SourceBal + "</td>"
   		         		+ "<td class='more'>" + value.debitClear_Des + "</td>"
   		                 + "</tr>";
   		              
   					$("#DClearList").append(eachrow);
      ...
      <tr>
        <td>
          <div class="headTableFloat">
            <strong>Debtor Clear Record</strong>
         		...
         		<div><button class="GrayBtn GrayBtn-primary" onclick="return SelectMailDynamic('Direct', 'DebitClearRecord');">Send email</button></div>
         		<div><button class="GrayBtn GrayBtn-primary" onclick="return commonPOPUpTwoDates('DebitClearRecord');">Excel</button></div>
          </div>
        </td>
      </tr>
      <tr>
          ...
          <form action="deleteBank" method="post" name="admin" id="myForm">
            <table border="1" id="example" class="display table table-striped table-bordered"  cellspacing="0" width="100%">
              ...
              <tr>
                ...
                <th style="border-right-style: hidden;">Debtor name</th>
